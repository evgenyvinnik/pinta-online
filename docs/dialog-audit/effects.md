...
Current closure: the shared web dialog now implements those compound/reset rows, live preview, scrollable content with pinned actions, native angle orientation (0° right, positive counter-clockwise, Shift snapping), and image-backed pixel-coordinate Point/CenterOffset pickers. Processor-facing center offsets remain normalized internally and are converted only at the dialog boundary.
