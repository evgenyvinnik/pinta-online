 ...
 * TypeScript module. Vitest loads it directly, so every assertion below is the original. The
 * fixtures come from a standalone C# transcription of the native Render methods and must never
...
const nativeEffects = nativeFixtures.effects;
...
describe('effect processors', () => {
  ...
  // These four fixtures come from tests/fixtures/native-effects.json, which is produced by
  // tools/effect-fixtures running the real C# effects out of original/. They pin Cairo
  // premultiplication, integer division, float bilinear weights, fixed-point stepping, skipped
  // out-of-bounds samples and transparent pixels. `npm run verify:native-fixtures` regenerates
  ...
  it('matches native Render output for the four premultiplied-sampling effects', () => {
    const cases: Array<[keyof typeof nativeEffects, EffectParameters, string]> = [
      ...
      [
        'motion-blur',
        { angle: 25, distance: 3, centered: 1 },
        'Motion Blur must match native point count, float bilinear weights, and empty-edge fallback',
      ],
      [
        'radial-blur',
        { angle: 27, offsetX: -0.2, offsetY: 0.25, quality: 2 },
        'Radial Blur must match native fixed-point stepping and skipped out-of-bounds samples',
      ],
      [
        'zoom-blur',
        { amount: 35, offsetX: 0.2, offsetY: -0.25 },
        'Zoom Blur must match native sample count, weighting, and transparent-source handling',
      ],
    ];

    for (const [effect, parameters, message] of cases) {
      assert.deepEqual(
  ...
  it('matches native output for seeded, translucent and gradient routines', () => {
    assert.deepEqual(
  ...
  it('keeps the Glow brightness adjustment off the finished composite', () => {
  ...
  it('indexes brightness and contrast by luminance, not per channel', () => {
  ...
  it('posterizes onto the native running-counter buckets', () => {
  ...
  it('truncates fixed-point luminance the way GetIntensityByte does', () => {
    assert.deepEqual(
