...
export function createOpenRasterStackXml(
  width: number,
  height: number,
  layersTopToBottom: Omit<OpenRasterLayerData, 'png'>[],
) {
  const layerXml = layersTopToBottom.map(
    (layer, index) =>
...
export function encodeOpenRasterArchive(data: OpenRasterData) {
  ...
  topToBottom.forEach((layer, index) => {
    files[`data/layer${index}.png`] = layer.png;
  });
...
export function decodeOpenRasterArchive(bytes: Uint8Array): OpenRasterData {
  ...
  if (!Number.isFinite(width) || !Number.isFinite(height) || width < 1 || height < 1)
    throw new Error('The OpenRaster image dimensions are invalid.');
