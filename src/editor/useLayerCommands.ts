...
export function useLayerCommands({
  ...
  const activeLayer = useCallback(
    () => layersRef.current.find((layer) => layer.id === activeLayerIdRef.current),
  ...
  const addLayer = useCallback(() => {
    ...
    const layer = makeLayer(
      ...
      `Layer ${layersRef.current.length + 1}`,
    );
  ...
  const importLayerFromFile = useCallback(
    async (file: File) => {
      commitPendingEditsRef.current();
  ...
  const flipLayer = useCallback(
    (direction: 'horizontal' | 'vertical') => {
      ...
      const next = layersRef.current.map((candidate) =>
        candidate.id === layer.id ? { ...candidate, canvas } : candidate,
      );
      setLayerList(next);
  ...
  const previewLayerProperties = useCallback(
    (layerId: string, properties: { visible: boolean; opacity: number; blendMode: BlendMode }) => {
      const preview = previewCanvasRef.current;
  ...
  const previewRotateZoomLayer = useCallback(
    (layerId: string, angle: number, panHorizontal: number, panVertical: number, zoomAmount: number) => {
      const preview = previewCanvasRef.current;
  ...
  const rotateZoomLayer = useCallback(
    (angle: number, panHorizontal: number, panVertical: number, zoomAmount: number) => {
  ...
  const updateLayerProperties = useCallback(
    (id: string, properties: { name: string; visible: boolean; opacity: number; blendMode: BlendMode }) => {
      commitPendingEditsRef.current();
