...
export function NativeToolOptions({
  editor,
  currentTool,
  blockBrushEnabled,
  onChooseFont,
  ...
  blockBrushEnabled: boolean;
  onChooseFont: () => void;
}) {
  const antialias = (
    <ToolbarIconSelect
      label="Antialiasing"
      value={editor.shapeAntialiasing ? 'on' : 'off'}
      options={ANTIALIAS_OPTIONS}
      onChange={(value) => editor.setShapeAntialiasing(value === 'on')}
    />
  );
  ...
  const selectionMode = (
    <ToolbarIconSelect
      ...
      options={SELECTION_MODE_OPTIONS.map((mode) => ({
        value: mode.value,
        label: translateUi(mode.label).replace('{0}', mode.value === 'intersect' ? alternateModifier : primaryModifier),
      }))}
      onChange={(value) => editor.setSelectionMode(value as SelectionMode)}
    />
  );
  const fillStyle = (
    <ToolbarIconSelect
      label="Fill style"
      value={editor.shapeFillStyle}
      options={FILL_STYLE_OPTIONS}
      onChange={(value) => editor.setShapeFillStyle(value as ShapeFillStyle)}
    />
  );
  const dash = (
    <>
      <input
        className="native-toolbar-select dash-option-select"
        list="pinta-dash-patterns"
        value={editor.shapeDashStyle}
        onChange={(event) => editor.setShapeDashStyle(event.target.value as ShapeDashStyle)}
        aria-label={translateUi('Dash pattern')}
      />
      <datalist id="pinta-dash-patterns">
        {['-', ' -', ' --', ' ---', '  -', '   -', ' - --', ' - - --------', ' - - ---- - ----'].map((pattern) => (
  ...
  const blend = (
    <ToolbarIconSelect
      label="Blending"
      value={editor.alphaBlendingMode}
      options={BLENDING_OPTIONS}
      onChange={(value) => editor.setAlphaBlendingMode(value as typeof editor.alphaBlendingMode)}
    />
  );
    ...
    <div className="tool-options-bar">
          ...
          {editor.tool === 'paintbrush' && (
            <>
              ...
              <ToolbarIconSelect
                ...
                label="Paintbrush type"
                showLabel
                value={editor.paintBrushType}
                options={[
              ...
              />
              {editor.paintBrushType === 'slash' && (
                <>
                  <span className="option-label">{translateUi('Angle:')}</span>
                  <ToolbarStepper
                    label="Slash angle"
                    value={editor.slashBrushAngle}
                    min={0}
                    max={180}
                    onChange={editor.setSlashBrushAngle}
                  />
                </>
              )}
              {editor.paintBrushType === 'splatter' && (
                <>
                  <span className="option-label">{translateUi('Minimum Size:')}</span>
                  <ToolbarStepper
                    label="Splatter minimum size"
                    value={editor.splatterMinimumSize}
                    min={1}
                    max={10000}
                    onChange={editor.setSplatterMinimumSize}
                  />
                  <span className="option-label">{translateUi('Maximum Size:')}</span>
                  <ToolbarStepper
                    label="Splatter maximum size"
                    value={editor.splatterMaximumSize}
                    min={1}
                    max={10000}
                    onChange={editor.setSplatterMaximumSize}
                  />
                </>
              )}
            </>
          ...
          {editor.tool === 'eraser' && (
            <>
              ...
              <ToolbarIconSelect
                ...
                label="Eraser type"
                showLabel
                value={editor.eraserType}
                options={[
                  { value: 'normal', label: 'Normal' },
                  { value: 'smooth', label: 'Smooth' },
                ]}
                onChange={(value) => editor.setEraserType(value as typeof editor.eraserType)}
              />
            </>
          )}
          {editor.tool === 'recolor' && (
            <>
              ...
              <output className="native-toolbar-value">{editor.recolorTolerance}</output>
              <input
                className="tool-option-slider compact"
                type="range"
                min="0"
                max="100"
                value={editor.recolorTolerance}
                onChange={(event) => editor.setRecolorTolerance(Number(event.target.value))}
                aria-label={translateUi('Recolor tolerance')}
              />
            </>
          )}
          {antialias}
      ...
      {['paint-bucket', 'magic-wand'].includes(editor.tool) && (
        <>
          ...
          <input
            ...
            min="0"
            max="100"
            value={editor.tool === 'magic-wand' ? editor.magicWandTolerance : editor.paintBucketTolerance}
            onChange={(event) =>
              editor.tool === 'magic-wand'
                ? editor.setMagicWandTolerance(Number(event.target.value))
                : editor.setPaintBucketTolerance(Number(event.target.value))
            }
            aria-label={translateUi('Tolerance')}
          />
          {editor.tool === 'magic-wand' && (
            <>
              <span className="option-label">{translateUi('Selection Mode:')}</span>
              {selectionMode}
            </>
          )}
        </>
      ...
      {['rectangle-select', 'ellipse-select', 'lasso-select'].includes(editor.tool) && (
        <>
          ...
          {editor.tool === 'lasso-select' ? (
            ...
            <ToolbarIconSelect
              label="Auto-scroll"
              value={editor.selectionAutoScroll ? 'on' : 'off'}
              options={[
      ...
      {shapeTool && (
        <>
          ...
          />
          {editor.tool === 'rounded-rectangle' && (
            <>
              <span className="option-label">{translateUi('Radius:')}</span>
              <ToolbarStepper
                label="Radius"
                value={editor.roundedRectangleRadius}
                min={0}
                max={100000}
                onChange={editor.setRoundedRectangleRadius}
              />
            </>
          )}
          <span className="option-label">{translateUi('Fill Style:')}</span>
          {fillStyle}
          {editor.shapeFillStyle !== 'fill' && (
            <>
              <span className="option-label">{translateUi('Outline width:')}</span>
              <ToolbarStepper
                label="Outline width"
                value={editor.brushSize}
                min={1}
                ...
                onChange={editor.setBrushSize}
              />
              <span className="option-label">{translateUi('Dash:')}</span>
              {dash}
            </>
          )}
          {editor.tool === 'line' && (
            <>
              <span className="option-label">{translateUi('Arrow:')}</span>
              <label className="native-toolbar-check">
                <input
                  aria-label={translateUi('Start arrow')}
                  type="checkbox"
                  checked={editor.lineArrowStart}
                  onChange={(event) => editor.setLineArrowStart(event.target.checked)}
                />
                1
              </label>
              <label className="native-toolbar-check">
                <input
                  aria-label={translateUi('End arrow')}
                  type="checkbox"
                  checked={editor.lineArrowEnd}
                  onChange={(event) => editor.setLineArrowEnd(event.target.checked)}
                />
                2
              </label>
              {(editor.lineArrowStart || editor.lineArrowEnd) && (
                <>
                  <span className="option-label">{translateUi('Size:')}</span>
                  <ToolbarStepper
                    label="Arrow size"
                    value={editor.lineArrowSize}
                    min={1}
                    max={100}
                    onChange={editor.setLineArrowSize}
                  />
                  <span className="option-label">{translateUi('Angle:')}</span>
                  <ToolbarStepper
                    label="Arrow angle"
                    value={editor.lineArrowAngle}
                    min={-89}
                    max={89}
                    onChange={editor.setLineArrowAngle}
                  />
                  <span className="option-label">{translateUi('Length:')}</span>
                  <ToolbarStepper
                    label="Arrow length"
                    value={editor.lineArrowLength}
                    min={-100}
                    max={100}
                    onChange={editor.setLineArrowLength}
                  />
                </>
              )}
            </>
        ...
        <>
          ...
          <ToolbarIconSelect
            ...
            options={[
              ...
              {
                value: 'reflected',
                label: 'Linear Reflected Gradient',
                icon: 'tool-gradient-linear-reflected-symbolic.svg',
              },
          ...
          <ToolbarIconSelect
            label="Gradient mode"
            value={editor.gradientColorMode}
            options={[
              { value: 'color', label: 'Color Mode', icon: 'tool-gradient-colormode-color-symbolic.svg' },
              {
                value: 'transparency',
                label: 'Transparency Mode',
                icon: 'tool-gradient-colormode-transparency-symbolic.svg',
              },
            ]}
      ...
      {editor.tool === 'color-picker' && (
        <>
          ...
          <ToolbarIconSelect
            label="Sampling size"
            showLabel
            value={String(editor.colorPickerSampleSize)}
            options={[
          ...
          <ToolbarIconSelect
            label="Sample source"
            showLabel
            value={editor.colorPickerSampleType}
            options={[
          ...
          <ToolbarIconSelect
            label="After select"
            showLabel
            value={editor.colorPickerAfterSelect}
            options={[
      ...
      {editor.tool === 'text' && (
        <>
          ...
          <button
            type="button"
            className="native-toolbar-select font-family-select"
            onClick={onChooseFont}
            aria-label={translateUi('Font family')}
            title={translateUi('Choose an installed font family')}
          >
            {editor.textFontFamily}
          </button>
          <ToolbarIconSelect
            label="Font variant"
            value={editor.textVariant}
            options={[
          ...
          <ToolbarStepper
            ...
            min={1}
            max={2000}
            onChange={editor.setTextFontSize}
          />
          <ToolbarIconSelect
            label="Font weight"
            value={String(editor.textFontWeight)}
            options={[
          ...
          />
          <button
            className={`text-format-button ${editor.textItalic ? 'active' : ''}`}
            type="button"
            aria-label={translateUi('Italic')}
            onClick={() => editor.setTextItalic(!editor.textItalic)}
          >
            <PintaIcon file="format-text-italic-symbolic.svg" size={15} standard />
          </button>
          <button
            className={`text-format-button ${editor.textUnderline ? 'active' : ''}`}
            type="button"
            aria-label={translateUi('Underline')}
            onClick={() => editor.setTextUnderline(!editor.textUnderline)}
          >
            <PintaIcon file="format-text-underline-symbolic.svg" size={15} standard />
          </button>
          {(
            [
              ['left', 'format-justify-left-symbolic.svg', 'Left align'],
              ['center', 'format-justify-center-symbolic.svg', 'Center align'],
              ['right', 'format-justify-right-symbolic.svg', 'Right align'],
            ] as const
          ).map(([alignment, icon, label]) => (
            <button
              key={alignment}
              className={`text-format-button ${editor.textAlignment === alignment ? 'active' : ''}`}
              type="button"
              aria-label={translateUi(label)}
              onClick={() => editor.setTextAlignment(alignment as TextAlignment)}
            >
              <PintaIcon file={icon} size={15} standard />
            </button>
          ))}
          <span className="option-label">{translateUi('Text Style:')}</span>
          <ToolbarIconSelect
            label="Text style"
            value={editor.textStyle}
            options={[
          ...
          {(editor.textStyle === 'fill-outline' || editor.textStyle === 'outline') && (
            <>
              ...
              <ToolbarStepper
                ...
                value={editor.textOutlineWidth}
                min={1}
                max={100000}
                onChange={editor.setTextOutlineWidth}
              />
              <span className="option-label">{translateUi('Join:')}</span>
              <ToolbarIconSelect
                className="text-join-select"
                label="Text outline join"
                showLabel
                value={editor.textLineJoin}
                options={[
                  { value: 'miter', label: 'Miter Join' },
                  { value: 'round', label: 'Round Join' },
                  { value: 'bevel', label: 'Bevel Join' },
                ]}
                onChange={(value) => editor.setTextLineJoin(value as CanvasLineJoin)}
              />
            </>
          )}
          {antialias}
