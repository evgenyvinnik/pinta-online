...
export function KeyboardShortcutsDialog({ onClose }: { onClose: () => void }) {
  ...
  const visibleSections = sections.flatMap((section) => {
    const entries = normalizedQuery
      ? section.entries.filter(([label, shortcut]) =>
          `${translateUi(label)} ${shortcut}`.toLocaleLowerCase().includes(normalizedQuery),
        )
      : section.entries;
    ...
    >
      <div className="pinta-dialog shortcuts-dialog" role="dialog" aria-modal="true" aria-labelledby="shortcuts-title">
        <header className="dialog-header shortcuts-header">
          <button
            type="button"
            className={`about-header-button ${searching ? 'active' : ''}`}
            aria-label={translateUi('Search shortcuts')}
          ...
          >
            <PintaIcon file="system-search-symbolic.svg" size={15} standard />
          </button>
          <strong id="shortcuts-title">{translateUi('Keyboard Shortcuts')}</strong>
          <button type="button" className="about-header-button" aria-label={translateUi('Close')} onClick={onClose}>
            ×
          </button>
        </header>
        {searching && (
          <div className="shortcuts-search">
            <input
              ...
              placeholder={translateUi('Search shortcuts')}
              aria-label={translateUi('Search shortcuts')}
            />
          </div>
        )}
          ...
          <nav className="shortcuts-navigation" aria-label={translateUi('Shortcut sections')}>
            {sections.map((section) => (
              <button
                ...
                onClick={() =>
                  document
                    .getElementById(`shortcut-section-${section.title.toLowerCase()}`)
          ...
          <div className="dialog-content shortcuts-content">
            {visibleSections.map((section) => (
              ...
              >
                ...
                <div className="shortcut-list">
                  {section.entries.map(([label, shortcut]) => (
            ...
            {!visibleSections.length && (
              <div className="shortcuts-empty">
                <PintaIcon file="system-search-symbolic.svg" size={34} standard />
                <strong>{translateUi('No shortcuts found')}</strong>
              </div>
            )}
          </div>
        </div>
...
export function LanguageDialog({ onClose }: { onClose: () => void }) {
  ...
  return (
    ...
    >
      <form
        className="pinta-dialog language-dialog"
        role="dialog"
        aria-modal="true"
      ...
      >
        <header className="dialog-header">
          <button type="button" className="dialog-text-button" onClick={onClose}>
            {translateUi('Cancel')}
          </button>
          <strong id="language-title">{translateUi('Choose language')}</strong>
          <button type="submit" className="dialog-text-button suggested">
            {translateUi('Apply')}
          </button>
        </header>
...
export function AddinManagerDialog({ enabledAddins, onToggle, onSetAll, onClose }: AddinManagerDialogProps) {
  ...
  return (
    ...
    >
      <div
        className="pinta-dialog addin-manager-dialog"
        role="dialog"
        aria-modal="true"
        aria-labelledby="addin-manager-title"
      >
        <header className="addin-manager-header">
          <button type="button" className="icon-button" disabled aria-label="Install Extension Package">
            <PintaIcon file="document-open-symbolic.svg" size={17} standard />
          </button>
          <button type="button" className="icon-button" aria-label="Refresh add-ins">
            <PintaIcon file="view-refresh-symbolic.svg" size={17} standard />
          </button>
        ...
        <nav className="addin-manager-tabs" aria-label="Add-in sections">
          {(
            ...
            >
              {translateUi(label)}
              {id === 'installed' && <small>{enabledCount}</small>}
            </button>
          ))}
        </nav>
          ...
          <div className="addin-manager-list-pane">
            <div className="addin-manager-actions">
              <button type="button" onClick={() => onSetAll(true)}>
                {translateUi('Enable all')}
              </button>
              <button type="button" onClick={() => onSetAll(false)}>
                {translateUi('Disable all')}
              </button>
            </div>
            <div className="addin-list" role="listbox" aria-label={translateUi('Add-ins')}>
              {listedAddins.map((addin) => (
                <button
                  key={addin.id}
                  type="button"
                  role="option"
                  aria-selected={selected?.id === addin.id}
                  onClick={() => setSelectedId(addin.id)}
                >
                  <strong>{translateUi(addin.name)}</strong>
                  <span>{translateUi(addin.description)}</span>
                </button>
              ))}
              {!listedAddins.length && (
                <div className="addin-empty">
                  <PintaIcon file="system-search-symbolic.svg" size={34} standard />
                  <strong>{translateUi('No Items Found')}</strong>
                </div>
              )}
            </div>
          </div>
          {selected && (
            <article className="addin-detail-pane">
              <span className="addin-manager-icon">
                <PintaIcon file="addins-manage.png" size={30} />
              </span>
              <h2>{translateUi(selected.name)}</h2>
              <span>
                v{selected.version} · {selected.author}
              </span>
              <p>{translateUi(selected.description)}</p>
              <div className="addin-capabilities">
                {selected.capabilities.map((capability) => (
                  <span key={capability}>{translateUi(capability)}</span>
                ))}
              </div>
              <footer>
                <label>
                  <span>{enabledAddins.includes(selected.id) ? translateUi('Enabled') : translateUi('Disabled')}</span>
                  <span className="addin-switch">
                    <input
                      type="checkbox"
                      checked={enabledAddins.includes(selected.id)}
                      onChange={(event) => onToggle(selected.id, event.target.checked)}
                    />
                    <span aria-hidden="true" />
                  </span>
                </label>
                <a href={selected.sourceUrl} target="_blank" rel="noreferrer">
                  {translateUi('More Information')} ↗
                </a>
              </footer>
              <small>
                {translateUi(selected.license)} ·{' '}
                {translateUi('Bundled with Pinta Online; no code is downloaded at runtime.')}
              </small>
            </article>
          )}
        </div>
...
export function AboutDialog({ onClose }: { onClose: () => void }) {
  ...
  const title =
    page === 'overview' ? 'About Pinta' : page === 'details' ? 'Details' : page === 'credits' ? 'Credits' : 'Legal';
  return (
    <div
      className="dialog-backdrop"
      role="presentation"
      ...
      <div className="pinta-dialog about-dialog" role="dialog" aria-modal="true" aria-labelledby="about-title">
        ...
        {page === 'overview' && (
          <div className="dialog-content about-content">
            ...
            <h2>Pinta</h2>
            <p className="about-version" data-visual-version>
              Pinta Online {__PINTA_ONLINE_VERSION__} · based on Pinta 3.2
            </p>
            <p>{translateUi('Easily create and edit images, now in the browser.')}</p>
            <p className="about-port-credit">
              {translateUi('Ported to the web by')}{' '}
              <a href={WEB_REPOSITORY_URL} target="_blank" rel="noreferrer">
                Evgeny Vinnik
              </a>
              .
            </p>
            <div className="about-links">
              <button type="button" onClick={() => setPage('details')}>
                <span>{translateUi('Details')}</span>
                <b>›</b>
              </button>
              <a href={USER_GUIDE_URL}>
                <span>{translateUi('Support Questions')}</span>
                <b>›</b>
              </a>
              <a href={WEB_BUG_REPORT_URL} target="_blank" rel="noreferrer">
                <span>{translateUi('Report an Issue')}</span>
                <b>›</b>
              </a>
              <button type="button" onClick={() => setPage('credits')}>
                <span>{translateUi('Credits')}</span>
                <b>›</b>
              </button>
              <button type="button" onClick={() => setPage('legal')}>
                <span>{translateUi('Legal')}</span>
                <b>›</b>
              </button>
            </div>
        ...
        {page === 'details' && (
          <article className="dialog-content about-subpage">
            ...
            <p>
              {translateUi(
                'Pinta Online brings the familiar Pinta painting and image-editing experience to modern web browsers.',
              )}
            </p>
            <dl>
              <div>
                <dt>{translateUi('Version')}</dt>
                <dd>Pinta Online {__PINTA_ONLINE_VERSION__}</dd>
              </div>
              <div>
                <dt>{translateUi('Based on')}</dt>
                <dd>Pinta 3.2</dd>
              </div>
              <div>
                <dt>{translateUi('Website')}</dt>
                <dd>
                  <a href={aboutPathForLocale(currentLocale())} aria-label={translateUi('Website')}>
                    {translateUi('Pinta Online website')}
                  </a>
                </dd>
              </div>
              <div>
                <dt>{translateUi('Source Code')}</dt>
                <dd>
                  <a href={WEB_REPOSITORY_URL} target="_blank" rel="noreferrer" aria-label={translateUi('Source Code')}>
                    github.com/evgenyvinnik/pinta-online
                  </a>
                </dd>
              </div>
            </dl>
        ...
        {page === 'credits' && (
          <article className="dialog-content about-subpage about-credits">
            ...
            <section>
              ...
              <ul>
                {PINTA_DEVELOPERS.map((developer) => (
                  <li key={developer}>{developer}</li>
                ))}
              </ul>
              <a href="https://github.com/PintaProject/Pinta/graphs/contributors" target="_blank" rel="noreferrer">
                {translateUi('View all Pinta contributors')} ↗
              </a>
            </section>
        ...
        {page === 'legal' && (
          <article className="dialog-content about-subpage about-legal">
            ...
            <p>
              <a href={`${WEB_REPOSITORY_URL}/blob/master/LICENSE`} target="_blank" rel="noreferrer">
                {translateUi('Read the complete license')} ↗
              </a>
            </p>
...
export function FontFamilyDialog({
  families,
  current,
  onCancel,
  onSubmit,
  ...
  return (
    ...
    >
      <form
        className="pinta-dialog font-family-dialog"
        role="dialog"
        aria-modal="true"
        aria-labelledby="font-family-dialog-title"
        onSubmit={(event) => {
          event.preventDefault();
          onSubmit(selected);
        }}
      >
        <h2 className="visually-hidden" id="font-family-dialog-title">
          Choose Font Family
        </h2>
        <div className="font-family-dialog-content">
          ...
          <div className="font-family-preview" style={{ fontFamily: `"${selected}"` }}>
