...
export interface ImageSizeDialogProps {
  ...
  onSubmit: (
    ...
    height: number,
    anchor: CanvasAnchor,
    background: 'white' | 'secondary' | 'transparent',
    resampling: string,
  ) => void;
}
...
function loadResizeSettings(
  mode: 'resize-image' | 'resize-canvas',
  width: number,
  height: number,
): StoredResizeSettings {
  ...
  try {
    const stored = JSON.parse(
      localStorage.getItem(`pinta-online-${mode}-settings`) ?? 'null',
    ) as Partial<StoredResizeSettings> | null;
    ...
    return {
      ...
      percentage: Number.isFinite(stored.percentage)
        ? Math.max(1, Math.min(10000, Math.round(stored.percentage!)))
        : 100,
...
export function ImageSizeDialog({
  ...
  if (mode === 'new') {
    ...
    return (
      ...
      >
        <form
          className="pinta-dialog native-new-image-dialog"
          role="dialog"
          aria-modal="true"
          aria-labelledby="image-size-title"
          onSubmit={(event) => {
            event.preventDefault();
            onSubmit(width, height, anchor, background, resampling);
          }}
        >
          <h2 className="visually-hidden" id="image-size-title">
          ...
          <div className="native-new-image-content">
            <section className="native-new-image-options" aria-label={translateUi('Image options')}>
              ...
              <fieldset className="native-choice-group native-background-group">
                ...
                <label>
                  <i className="native-color-swatch" style={{ background: '#ffffff' }} />
                  <input
                    type="radio"
                    name="background"
                  ...
                  />
                  <span>{translateUi('White')}</span>
                </label>
                {secondaryColor.toLowerCase() !== '#ffffff' && (
                  <label>
                    <i className="native-color-swatch" style={{ background: secondaryColor }} />
                    <input
                      type="radio"
                      name="background"
                      checked={background === 'secondary'}
                      onChange={() => setBackground('secondary')}
                    />
                    <span>{translateUi('Background Color')}</span>
                  </label>
                )}
                <label>
                  <i className="native-color-swatch checkerboard" />
                  <input
                    type="radio"
                    name="background"
                  ...
                  />
                  <span>{translateUi('Transparent')}</span>
                </label>
              </fieldset>
            </section>
            <section className="native-new-image-preview-wrap" aria-label={translateUi('Preview')}>
              ...
              <div
                className={`native-new-image-preview ${background === 'transparent' ? 'checkerboard' : ''}`}
                style={{
                  aspectRatio: `${width} / ${height}`,
                  backgroundColor: background === 'transparent' ? undefined : previewBackground,
                }}
              />
            </section>
          </div>
          <footer className="native-dialog-actions">
            ...
            <button type="submit" className="native-dialog-button suggested">
              {translateUi('OK')}
            </button>
          </footer>
        </form>
  ...
  return (
    ...
    >
      <form
        className={`pinta-dialog native-resize-dialog ${mode === 'resize-canvas' ? 'native-resize-canvas-dialog' : ''}`}
        role="dialog"
        aria-modal="true"
        aria-labelledby="image-size-title"
        onSubmit={(event) => {
          ...
          localStorage.setItem(
            ...
            JSON.stringify({
              ...
              anchor,
              resampling,
            } satisfies StoredResizeSettings),
          );
          onSubmit(width, height, anchor, background, resampling);
        }}
      >
        ...
        <div className="native-resize-content">
          <label className="native-radio-row percentage-row">
            <input
              type="radio"
              name="size-mode"
              checked={sizeMode === 'percentage'}
              onChange={() => setSizeMode('percentage')}
            />
          ...
          <label className="native-radio-row absolute-row">
            <input
              type="radio"
              name="size-mode"
              checked={sizeMode === 'absolute'}
              onChange={() => setSizeMode('absolute')}
            />
          ...
          <div className="native-size-grid">
            ...
            <DialogStepper
              ...
              disabled={sizeMode !== 'absolute'}
            />
            <i>{translateUi('pixels')}</i>
            <DialogResetButton
              label="Reset to image size"
              disabled={sizeMode !== 'absolute'}
              onClick={() => {
          ...
          <label className="native-check-row">
            ...
            />
            <span>{translateUi('Maintain aspect ratio')}</span>
          </label>
          {mode === 'resize-image' && (
            <label className="native-resampling-row">
              ...
              <select
                ...
                aria-label={translateUi('Resampling')}
              >
              ...
              <div className="native-anchor-picker">
                {ANCHORS.map((item) => (
                  <button
                    key={item}
                    type="button"
                    aria-label={`${translateUi(item)} ${translateUi('anchor')}`}
