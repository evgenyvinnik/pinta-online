...
export function LayerPropertiesDialog({ layer, onCancel, onPreview, onSubmit }: LayerPropertiesDialogProps) {
  ...
  return (
    ...
    >
      <form
        className="pinta-dialog layer-properties-dialog"
        role="dialog"
        aria-modal="true"
      ...
      >
        <h2 className="visually-hidden" id="layer-properties-title">
          {translateUi('Layer Properties')}
        </h2>
        <div className="dialog-content layer-properties-content">
          <label className="layer-property-field">
            ...
            <input
              ...
              aria-label={translateUi('Layer name')}
            />
          </label>
          ...
          <label className="layer-property-field">
            ...
            <select
              ...
              aria-label={translateUi('Blend mode')}
            >
              {BLEND_MODES.map((mode) => (
                <option key={mode.id} value={mode.id}>
                  {translateUi(mode.label)}
                </option>
              ))}
            </select>
          </label>
          <label className="layer-opacity-field">
            ...
            <span className="layer-opacity-value">
              <input
                ...
                max="100"
                value={opacity}
                onChange={(event) => setOpacity(Math.max(0, Math.min(100, Number(event.target.value))))}
...
export function RotateZoomLayerDialog({
  layer,
  imageWidth,
  imageHeight,
  thumbnailUrl,
  ...
  thumbnailUrl: string;
  onCancel: () => void;
  onPreview: (layerId: string, angle: number, panHorizontal: number, panVertical: number, zoom: number) => void;
  onSubmit: (angle: number, panHorizontal: number, panVertical: number, zoom: number) => void;
}) {
  ...
  const updatePanX = (x: number) => {
    setPanX(x);
    setPanHorizontal((x * 2) / imageWidth - 1);
  };
  const updatePanY = (y: number) => {
    setPanY(y);
    setPanVertical((y * 2) / imageHeight - 1);
  };
  useEffect(() => {
  ...
  return (
    ...
    >
      <form
        className="pinta-dialog rotate-zoom-dialog"
        role="dialog"
        aria-modal="true"
        aria-labelledby="rotate-zoom-title"
        onSubmit={(event) => {
          event.preventDefault();
          onSubmit(angle, panHorizontal, panVertical, zoom);
        }}
      >
        <h2 className="visually-hidden" id="rotate-zoom-title">
          {translateUi('Rotate / Zoom Layer')}
        </h2>
        <div className="dialog-content rotate-zoom-content">
          <div className="native-transform-control">
            <strong>{translateUi('Angle')}</strong>
            <div>
              <AngleDial value={angle} min={-360} max={360} onChange={setAngle} />
              <DialogStepper label="Layer rotation angle" min={-360} max={360} value={angle} onChange={setAngle} />
              <DialogResetButton label="Reset angle" onClick={() => setAngle(0)} />
            </div>
          </div>
            ...
            <div>
              ...
              <span className="native-effect-point-fields">
                <label>
                  ...
                  />
                  <DialogResetButton
                    label="Reset horizontal pan"
                    onClick={() => updatePanX(Math.floor(imageWidth / 2))}
                  />
                </label>
                <label>
                  <span>Y:</span>
                  <DialogStepper
                    label="Layer vertical pan"
                    min={0}
                    max={imageHeight}
                    value={panY}
                    onChange={updatePanY}
                  />
                  <DialogResetButton
                    label="Reset vertical pan"
                    onClick={() => updatePanY(Math.floor(imageHeight / 2))}
                  />
                </label>
              </span>
            </div>
          </div>
          <label className="native-effect-range native-transform-zoom">
            <strong>{translateUi('Zoom')}</strong>
            <span>
              <input
                type="range"
                min="0"
              ...
              />
              <DialogStepper label="Layer zoom value" min={0} max={16} step={0.01} value={zoom} onChange={setZoom} />
              <DialogResetButton label="Reset zoom" onClick={() => setZoom(1)} />
            </span>
          </label>
        </div>
